...
var AnnotatorUI = (function ($, window, undefined) {
    var AnnotatorUI = function (dispatcher, svg) {
        ...
        //new add by sharad
        $('#svg').append('<div id = "summary_ctrl_div">' +
            '<button xmlns="http://www.w3.org/1999/xhtml" type="button" id="summ_field-ok" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button" aria-disabled="false"><span class="ui-button-text">Group Summaries</span></button>' +
            '<button xmlns="http://www.w3.org/1999/xhtml" type="button" id="delete_summaries" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button" aria-disabled="false"><span class="ui-button-text">Delete Summaries</span></button>' +
            '</div>');
        $('#summ_field-ok').on('click', function () {
            alert("class based grouping ");
            ...
            //
            spanOptions = {
                action: 'collateBasedOnEntity',
                collection: coll,
                'document': doc,
                'offsets' : JSON.stringify([1,1]),
                'type': "something",
                'id': -111
            };

            dispatcher.post('ajax', [spanOptions, function (response) { location.reload();}]);

            // dispatcher.post('ajax', [coll, doc]);
        });
        $('#delete_summaries').on('click', function () {
            alert("delete class based grouping ");
            spanOptions = {
                action: 'deleteSummaryGroups',
                collection: coll,
                'document': doc,
                'offsets' : JSON.stringify([1,1]),
                'type': "something",
        ...
        //  var onSummaryClick = function (evt) {
        //     // must be logged in
