...
def collate_on_entity(collection, document):
    #print collection
    import os.path
    #print os.path.isfile(os.getcwd()+"/data/"+collection +"/"+document+".ann")
    answer_path_list = [os.getcwd(), "data", collection, document+".txt"]
    annotation_path_list = [os.getcwd(), "data", collection, document + ".ann"]
    path = "/".join(annotation_path_list)
    answer_path = "/".join(answer_path_list)
    #print path
    lst = []
    summ = {}
    with open(path, "r") as annFile:
        lines = annFile.readlines()
        #lines_grouped_by_class  = _collate_by_entity()
        for i in lines:
            entry = i.split("\t") # split each line of the annotation file column wise
            entity = entry[1].split(" ")[0] # each line looks like 'T10\tQUESTION 0 8\turi : 18\n'
            selected_span = entry[2].strip() #remove newlines
            if entity in summ:
                summ[entity].append(selected_span)
            else:
                summ[entity] = []
                summ[entity].append(selected_span)

    with open(answer_path, "a") as answerFile:
        for key in summ.keys():
            if key != 'AnnotatorNotes':
                line = ".".join(summ[key])
                lst.append(line)
                answerFile.write(key+"_GROUP"+": " + line + "\n")
    #with open(answer_path_list, "a") as answerFile:
    #    for line in lst:
    #print lst
    return {}

def delete_grouped_spans(collection, document):
    #print collection
    import os.path
    #print os.path.isfile(os.getcwd()+"/data/"+collection +"/"+document+".ann")
    answer_path_list = [os.getcwd(), "data", collection, document+".txt"]
    ...
    answer_path = "/".join(answer_path_list)
    print path
    retainable_lines_annotator = []
    original_lines_ann = []
    retainable_lines_answer = []
    original_lines_answer = []
    summ = {}
    summ_annotation_id = ""
    with open(path, "r") as annFile:
        lines = annFile.readlines()
        ...
        for i in lines:
            entry = i.split("\t") # split each line of the annotation file column wise
            entity = entry[1].split(" ")[0] # look for lines that have the class indicating summary i.e. they end with the _GROUP summary, e.g. INFORMATION_GROUP

            if "_SUMMARY" in entity:
                summ_annotation_id = entry[0]
                continue
            elif "AnnotatorNotes" == entity:
                if summ_annotation_id in entry[1] and summ_annotation_id!="":
                    summ_annotation_id = ""
                    continue
                else:
                    retainable_lines_annotator.append(i)
            else:
                retainable_lines_annotator.append(i)
    with open(path, "w") as annFile:
        for i in retainable_lines_annotator:
            annFile.write(i)

    # try:
    with open(answer_path, "r") as answerFile:
        lines = answerFile.readlines()
        for i in lines:
            entry = i.split(":")  # split each line of the annotation file column wise
            entity = entry[0]  # each line looks like 'T10\tQUESTION 0 8\turi : 18\n'
            if ("_GROUP" not in entity) and ('\n' != entity): 
                retainable_lines_answer.append(i)
                continue
    retainable_lines_answer.append('\n')
    with open(answer_path, "w") as answerFile:
        for i in retainable_lines_answer:
            answerFile.write(i)
    # except Exception as ex:
    #     print "error in deleting annotations"
    return {}
    #with open(answer_path_list, "a") as answerFile:
    #    for line in lst:

def _collate_by_entity(text):
    return ""
